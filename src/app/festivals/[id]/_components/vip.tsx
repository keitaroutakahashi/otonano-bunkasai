...
export const Vip = ({ festival }: Props) => {
  if (!festival.vip) {
    return null;
  }
    ...
    <section>
      ...
      <div className="max-w-2xl mx-auto">
        <ul className="flex flex-wrap gap-8 justify-center">
          {festival.vip.map((vip) => (
            <li
              key={vip}
              className="border rounded-md border-slate-500 text-white md:py-5 py-3 px-8 relative"
            >
              ...
              <span className="font-bold md:text-lg text-base">{vip}</span>
            </li>
          ))}
        </ul>
      </div>
